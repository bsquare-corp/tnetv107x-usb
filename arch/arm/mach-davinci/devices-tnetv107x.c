...
int __init tnetv107x_register_usb20(struct musb_hdrc_platform_data platform_data[])
{
	int ret;
	int i;
	...
	for (i = 0; i <= 1; i++) {
		if (platform_data[i].mode != MUSB_UNDEFINED) {
			...
			usb_dev[i].resource = tnetv107x_usb20_resources[i];
			usb_dev[i].num_resources = ARRAY_SIZE(tnetv107x_usb20_resources[i]);
