...
#define TNETV107X_USB_STAT_REG	0x08
#define TNETV107X_USB_EMULATION_REG 0x0c
/* 0x10 reserved */
#define TNETV107X_USB_AUTOREQ_REG	0x14
#define TNETV107X_USB_SRP_FIX_TIME_REG 0x18
#define TNETV107X_USB_TEARDOWN_REG	0x1c
...
/* 0x5c reserved */
#define TNETV107X_USB_END_OF_INTR_REG 0x60

/*
 * TNETV107X specific definitions
 */

#define TNETV107X_TX_EP_MASK	0xffff		/* EP0 + 15 Tx EPs */
#define TNETV107X_RX_EP_MASK	0xfffe		/* 15 Rx EPs */

#define TNETV107X_USB_INTR_RX_SHIFT	16
#define TNETV107X_USB_INTR_TX_SHIFT	0

#define TNETV107X_TX_INTR_MASK	(TNETV107X_TX_EP_MASK << TNETV107X_USB_INTR_TX_SHIFT)
#define TNETV107X_RX_INTR_MASK	(TNETV107X_RX_EP_MASK << TNETV107X_USB_INTR_RX_SHIFT)

#define TNETV107X_USB_INTR_USB_SHIFT	16
#define TNETV107X_USB_INTR_USB_MASK	(0x1ff << TNETV107X_USB_INTR_USB_SHIFT) 
#define TNETV107X_USB_INTR_DRVVBUS		0x100

/* Control register bits */
#define TNETV107X_USB_SOFT_RESET_MASK	1

#define TNETV107X_MENTOR_CORE_OFFSET 0x400

#define TNETV107X_USB_PHY_CTRL		0x08087118
...
static inline int phy_ctrl(int controller, int turn_on)
{
	...
	phyreset = ioremap(TNETV107X_USB_PHY_RESET, 4);
	if (!phyreset) {
		pr_err("Unable to map USB PHY reset register\n");
		goto error0;
	}

	phyctl = ioremap(TNETV107X_USB_PHY_CTRL, 4);
	if (!phyctl) {
		pr_err("Unable to map USB PHY CTL register\n");
		goto error1;
	}

	spin_lock(&tnetv_lock);
	...
	if (turn_on) {
		 ...
		 */
		if (controller == 1) {
			phyctrl_val |= (USB1_PHYPLLON | USB1_SENSEEN | USB1_VBUSDETEN);
			phyctrl_val &= ~(USB1_PHYPWRDN | USB1_OTGPWRDN);
		} else if (controller == 0) {
			phyctrl_val |= (USB0_PHYPLLON | USB0_SENSEEN | USB0_VBUSDETEN);
			phyctrl_val &= ~(USB0_PHYPWRDN | USB0_OTGPWRDN);
		}
		...
		if (controller == 1) {
			while (!(__raw_readl(phyctl) & USB1_PHYCLKGD))
				cpu_relax();
		} else if (controller == 0) {
			while (!(__raw_readl(phyctl) & USB0_PHYCLKGD))
				cpu_relax();
		}
	} else {
...
void musb_platform_enable(struct musb *musb)
{
	...
	epmask = (((musb->epmask & TNETV107X_TX_EP_MASK) << TNETV107X_USB_INTR_TX_SHIFT) |
		((musb->epmask & TNETV107X_RX_EP_MASK) << TNETV107X_USB_INTR_RX_SHIFT));

	musb_writel(reg_base, EP_INTR_MASK_SET_REG, epmask);
	musb_writel(reg_base, CORE_INTR_MASK_SET_REG, TNETV107X_USB_INTR_USB_MASK);

	/* Force the DRVVBUS IRQ so we can start polling for ID change. */
	if (is_otg_enabled(musb))
		musb_writel(reg_base, CORE_INTR_SRC_SET_REG,
			    TNETV107X_USB_INTR_DRVVBUS << TNETV107X_USB_INTR_USB_SHIFT);
}
...
void musb_platform_disable(struct musb *musb)
{
	void __iomem *reg_base = musb->ctrl_base;
        ...
        musb_writel(reg_base, CORE_INTR_MASK_CLEAR_REG, TNETV107X_USB_INTR_USB_MASK);
        musb_writel(reg_base, EP_INTR_MASK_CLEAR_REG,
                         TNETV107X_TX_INTR_MASK | TNETV107X_RX_INTR_MASK);
        musb_writeb(musb->mregs, MUSB_DEVCTL, 0);
        musb_writel(reg_base, TNETV107X_USB_END_OF_INTR_REG, 0);
}
...
static void otg_timer(unsigned long _musb)
{
	...
	case OTG_STATE_A_WAIT_VFALL:
		...
		musb_writel(musb->ctrl_base, CORE_INTR_SRC_SET_REG,
			    MUSB_INTR_VBUSERROR << TNETV107X_USB_INTR_USB_SHIFT);
		break;
...
static irqreturn_t tnetv107x_interrupt(int irq, void *hci)
{
	...
	if (epintr) {
		...
		musb->int_rx =
			(epintr & TNETV107X_RX_INTR_MASK) >> TNETV107X_USB_INTR_RX_SHIFT;
		musb->int_tx =
			(epintr & TNETV107X_TX_INTR_MASK) >> TNETV107X_USB_INTR_TX_SHIFT;
	}
	...
	if (usbintr) {
		...
		musb->int_usb =
			(usbintr & TNETV107X_USB_INTR_USB_MASK) >> TNETV107X_USB_INTR_USB_SHIFT;
	}
	/*
	 ...
	 */
	if (usbintr & (TNETV107X_USB_INTR_DRVVBUS << TNETV107X_USB_INTR_USB_SHIFT)) {
		int drvvbus = musb_readl(reg_base, TNETV107X_USB_STAT_REG);
		void __iomem *mregs = musb->mregs;
...
int __init musb_platform_init(struct musb *musb, void *board_data)
{
	...
	musb->mregs += TNETV107X_MENTOR_CORE_OFFSET;

	usb_ss = clk_get(musb->controller, "clk_usbss");
	if (IS_ERR(usb_ss)) {
		pr_err("usb_ss clock failure\n");
		goto fail;
	}
	...
	rev = musb_readl(reg_base, TNETV107X_USB_REVISION_REG);
	if (!rev) {
	...
	pr_debug("TNETV107x OTG revision %08x, control %02x\n",
		 rev, musb_readb(reg_base, TNETV107X_USB_CTRL_REG));
