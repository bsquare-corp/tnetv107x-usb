...
struct cppi41 {
	...
	u16 teardown_q_num;		/* Teardown completion queue number */
...
static int __init cppi41_controller_start(struct dma_controller *controller)
{
	...
	atomic_add(1, &cppi->musbs);
	if (atomic_read(&cppi->musbs) != 1)
		return 1;

	spin_lock_init(&cppi->lock);
	/*
	...
	if (cppi41_queue_init(&cppi->queue_obj, 0, cppi->teardown_q_num)) {
 ...
 free_queue:
	if (cppi41_queue_free(0, cppi->teardown_q_num))
...
static void cppi41_autoreq_update(struct cppi41_channel *rx_ch, u8 autoreq)
{
	...
	if (is_host_active(rx_ch->end_pt->musb) &&
		...
		spin_lock_irqsave(&cppi->lock, flags);
		 reg_val = musb_readl(reg_base, USB_AUTOREQ_REG);
